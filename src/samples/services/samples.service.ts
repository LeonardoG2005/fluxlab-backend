...
export class SamplesService {
  ...
  async createManyWithValues(
    ...
    await this.ensureNoDuplicatedCodesInBulkRequest(
  ...
  private async ensureCodeUniquePerProject(
    ...
    if (existingSample) {
      throw new ConflictException(
        await this.buildCodeAlreadyExistsMessage(code, projectId),
      );
    }
  }

  private async buildCodeAlreadyExistsMessage(
    code: string,
    projectId: string,
  ): Promise<string> {
  ...
  private async getProjectNameMapByIds(
    projectIds: string[],
